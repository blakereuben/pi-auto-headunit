...
- **The USB write timeout is specific to a *second* proactive `PingRequest`, or to sending one while channel-setup traffic is concurrently flowing** — narrowed by the LIVI-timing trial (a first, correctly-timed ping succeeded; the second, sent while/after the full channel-open and key-binding sequence had run, did not). Not yet distinguished from ordinary USB-layer flakiness recurring at roughly that point in the session regardless of what's being sent.
...
1. **Done, refuted/no-change:** `ui_config` + USB write instrumentation were run together on real hardware (nineteenth run) — see "`ui_config` implemented; USB writes instrumented" above. `ui_config` shows no evidence of helping. The instrumentation did deliver real new data: the second-ping write genuinely blocks at the libusb level for the full 10s timeout, with nothing else in flight and the immediately preceding non-ping writes completing instantly — evidence leaning toward "ping-specific," not "any write at this session point," but not yet proof.
2. **Isolate ping-specific vs. write-position-specific**, now the best-motivated next step for the write-timeout question specifically: with the two controlled variants an external analysis suggested — (a) send a harmless non-ping control frame at the same point instead of the second ping, (b) suppress all other outbound traffic and send only pings. Not yet implemented as separate modes — a genuinely new pair of experiments, distinct from every fix attempted so far (items 7-8, 15 above).
3. **Does sending real (or even minimal/synthetic) media data after `Start` change anything?** Still the best-motivated theory for Error 2 itself if (1)/(2) don't resolve it — see "What this rules out, and what it doesn't". This is real M3 scope (encode/render pipeline: at minimum a single valid, decodable H.264 frame on the video channel, now that `Ack` handling exists to support it), materially larger than any prior increment — plan it as such.
