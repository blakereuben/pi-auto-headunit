...
Ruled out: a simple missing-channel gap; a simple version-number-field mismatch; a simple missing-identity gap; a too-short probe timeout (30s produced no change over 10s, so the phone isn't simply slow to continue — it appears to genuinely stop sending anything once it decides to fail); a missing sensor-capability advertisement; missing `NavFocusRequest`/`ByeByeRequest` handling; the ping write-timeout being caused by plain-vs-encrypted framing or too-short a transport timeout; and, most significantly, **a missing wire-protocol handshake step through `Start`** — every step through video `Start` (the deepest point Android Auto's wire protocol requires before real media begins) is now real-hardware-confirmed correct, including the previously-undiscovered unsolicited `VideoFocusNotification`.

**Not ruled out**, in roughly descending order of how well-motivated they are:

- **Missing real media data after `Start`.** This probe has never sent a single byte of actual video/audio stream data (`MEDIA_MESSAGE_DATA`) — deliberately out of scope since its inception, and separate M3 encode/render pipeline work that doesn't exist yet. Now that `Start` itself is reached, this is the new leading theory: a real phone plausibly times out or shows Error 2 specifically because no stream ever follows `Start`, not because of anything wrong with the handshake. Testing this requires actually streaming *something* (even a minimal/synthetic video frame) after `Start`, which is a materially larger scope step than anything tried so far — see "Suggested next steps" below.
- **A genuine 1.7 schema change** elsewhere in the protocol that this project has no source for. Weakened as a leading theory now that `Start` has been reached correctly, but not eliminated for whatever comes after it (media negotiation/ack behavior, `MEDIA_MESSAGE_ACK`, etc.).
...
1. **Immediate next question: does sending real (or even minimal/synthetic) media data after `Start` change anything?** This is now the single best-motivated next step — the entire wire-protocol handshake this probe implements has been real-hardware-confirmed correct through `Start` (see the `VideoFocusNotification` breakthrough above), and Error 2 still appeared with nothing ever following. This is a materially larger scope step than any prior increment: it means actually sending `MEDIA_MESSAGE_DATA` on the video channel (at minimum a single valid H.264 frame — doesn't need to be a real camera/screen feed to test the hypothesis, just *something* decodable), and very likely needs `MEDIA_MESSAGE_ACK` handling too (`Config.max_unacked` currently advertises `1`, meaning the phone may expect an ack before sending more than one unacknowledged frame — this project has never implemented ack handling at all, out of scope until now). This is real M3 scope (encode/render pipeline), not a small protocol tweak like every prior increment — plan it as such.
2. If (1) doesn't resolve Error 2 either, the same LIVI-comparison approach that found `VideoFocusNotification` should be repeated for whatever comes after `Start`/first media frame in LIVI's own `Session.ts`/related files — that file is now a proven, higher-signal reference than the 2018-era AASDK/`OpenAuto` sources for finding gaps a real modern phone actually cares about.
3. Diagnose the still-unresolved USB bulk-OUT write timeout (see "Ping write-timeout diagnosis" above) if continuing to work with `PingRequest` active — two direct fix attempts (encrypted framing, matching AASDK's 10s timeout) both failed to resolve it, and it's now suspected to reflect the phone itself abandoning the USB connection in earlier, less-complete runs rather than anything specific to how the ping is sent. This may simply stop occurring now that the session can complete successfully before a 5-second ping ever needs to fire, in which case it may not be worth further dedicated effort unless it recurs.
