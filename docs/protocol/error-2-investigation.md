...
- **The USB write timeout is specific to *any* second proactive, unsolicited write in the session — not to `PingRequest` or its content.** Now the leading theory for the write timeout specifically (distinct from the leading theory for Error 2 itself, above): every reactive write (a response to something the phone just sent) succeeds indefinitely throughout the session; only the second-and-later *head-unit-initiated* write ever fails, and it fails identically regardless of what it contains (confirmed with a genuinely different message type, "Ping isolation experiment"). Not yet explained *why* — a real phone plausibly only tolerates one proactive/unsolicited message per session before requiring something else (a response, an ack, a state transition) to "re-earn" the right to speak first again, but that's speculation pending further evidence.
...
1. **Done, refuted/no-change:** `ui_config` + USB write instrumentation, then the ping-isolation experiment, were all run on real hardware (nineteenth/twentieth runs). `ui_config` shows no evidence of helping. The write-timeout question is now resolved at the "what" level, though not the "why": it's a second proactive/unsolicited write of any kind, not `PingRequest` specifically — see "Ping isolation experiment" above.
2. **Find out *why* a second proactive write fails but every reactive write succeeds indefinitely.** The best-motivated remaining question for the write-timeout specifically. Candidate angles not yet tried: does a *third* proactive write (after more reactive traffic) succeed or also fail, i.e. is it strictly "the second one ever" or "any write not immediately preceded by inbound data"? Does inserting a deliberate reactive round-trip (wait for any phone-sent frame) immediately before the second proactive send change anything? Packet-capture-level USB analysis (e.g. `usbmon`) of exactly what the phone's side does around that write, if feasible on Pi 5, would settle this more directly than further black-box trials.
