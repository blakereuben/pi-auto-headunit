...
## Status: open, unresolved, but the failure boundary keeps moving further into the session with each increment. Three early hypotheses tested and refuted; advertising the full 8-service canonical set, then handling `AudioFocusRequest`, then `KeyBindingRequest` on the Input channel, then `Setup`/`Config`/`Start` on the `MediaAudio` and `SystemAudio` channels, each produced real forward progress — the phone now opens every channel, drives video and both audio channels through `Setup`→`Config`, and completes a full key-binding round trip — but Error 2 still appears, now triggered by a new, later, unhandled message on the `SpeechAudio` channel.
...
probe_state=encrypted_frame_received
error=protocol probe: unexpected message on channel 5 after open
```

**Confirmed on the phone screen: still Error 2** ("Communication error 2 — the phone and the car are running incompatible software"), despite the probe now getting substantially further than in every earlier attempt recorded below. This continues to confirm Error 2 is not a single fixed rejection point tied to one specific message — it re-appears at whatever point the session first does something the phone doesn't accept, and that point has now moved from "immediately after `ServiceDiscoveryResponse`" to "after all 8 channels are open, video/`MediaAudio`/`SystemAudio` have all received `Config`, and a `KeyBindingRequest`/`KeyBindingResponse` round trip on the Input channel has completed successfully."

The phone's `KeyBindingRequest` on the Input channel was empty (`key_binding_requested_count=0` — it requested zero keycodes), so the probe's `evaluate_key_binding_request` policy answered `Success` (an empty request has nothing unsupported in it) and the phone accepted that response and moved on, rather than erroring or retrying. The phone's `Setup` on both the `MediaAudio` and `SystemAudio` channels was likewise accepted without complaint once answered with a PCM `Config` — `SystemAudio`'s `Setup` used the same shared `AudioSetupStateMachine` as `MediaAudio`, unmodified, and the phone accepted it exactly the same way.

**New failure boundary:** channel 5 (`SpeechAudio`) receives a message after `ChannelOpenResponse` that the probe has no handler for — same shape as the previous `SystemAudio` boundary, just one channel later, and exactly the channel flagged as the next most likely candidate in the previous version of this document (also an `AudioMediaSinkService`-derived channel). `ChannelOpenStateMachine` only models advertise→open, nothing past `Open`, for any channel except `Video`/`MediaAudio`/`SystemAudio` (each has their own `Setup`/`Config`/`Start` state machine) and `Input` (has `KeyBindingRequest`/`Response` handling). Byte content of the unhandled message was not logged (no raw payloads, per this project's fail-closed instrumentation policy) — only that it arrived and on which channel.
...
- All 8 canonical services (`Video`, `Input`, `MediaAudio`, `SystemAudio`, `SpeechAudio`, `Sensors`, `Bluetooth`, `Microphone`) advertised in `ServiceDiscoveryResponse` (`crates/protocol-aap/src/service_discovery_response.rs`), each driven to `ChannelOpenState::Open` via `ChannelOpenStateMachine` in `auth_discovery_probe.rs`. `Video`, `MediaAudio`, and `SystemAudio` go further still (`Setup`→`Config`→`Start` via `VideoSetupStateMachine`/`AudioSetupStateMachine`), and `Input` handles one post-open message (`KeyBindingRequest`) — no other channel has any post-open handshake.
...
- `Setup`/`Config`/`Start` handling on the `MediaAudio` and `SystemAudio` channels (`crates/protocol-aap/src/audio_setup.rs`'s single `AudioSetupStateMachine`, reused unmodified for both; `handle_media_audio_channel_message`/`handle_system_audio_channel_message` in `auth_discovery_probe.rs`) — accepts only `MEDIA_CODEC_AUDIO_PCM`, matching the single uncompressed `AudioConfiguration` this project advertises for each. Real-hardware-confirmed on both channels: each `Setup` requested PCM, each `Config` response was accepted, and the session proceeded past both boundaries in turn.
...
1. **Immediate next boundary:** figure out what message the phone sends on the `SpeechAudio` channel (channel 5) right after it opens, following the same successful pattern as `AudioFocusRequest`, `KeyBindingRequest`, and `MediaAudio`/`SystemAudio`'s `Setup`/`Config`/`Start`. `SpeechAudio` also derives from AASDK's `AudioMediaSinkService` (same base class as `MediaAudio`/`SystemAudio`, both of which turned out to need nothing more than reusing the existing `AudioSetupStateMachine`), so the same `Setup`/`Config`/`Start` handshake accepting `MEDIA_CODEC_AUDIO_PCM` is the leading expectation — but confirm this directly against the pinned source before writing any code, and check what `AudioConfiguration` `SpeechAudio` advertises in `auth_discovery_probe.rs`'s `build_service_capabilities` today (it already uses the same `VOICE_AUDIO_*` constants as `SystemAudio`/microphone, so PCM is likely but should be confirmed, not assumed). `ChannelOpenStateMachine` has no state past `Open` for `SpeechAudio`; this is expected to need its own dispatch wiring (very possibly zero new crate-level code, same as `SystemAudio`).
2. Given the pattern so far (each unblocked boundary reveals exactly one more previously-unseen message type, and Error 2 keeps re-appearing at the new edge rather than disappearing), expect this to continue: handling the `SpeechAudio` message may well reveal a next message on another channel (`Sensors`, `Bluetooth`, or `Microphone` are the remaining candidates once all three `AudioMediaSinkService`-derived channels are handled). Budget for several more minimal, reversible increments rather than expecting one fix to resolve it.
