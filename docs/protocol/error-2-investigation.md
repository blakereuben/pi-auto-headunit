...
5. **Missing sensor-capability advertisement.** `ServiceDiscoveryResponse.Sensors` had never advertised any supported `SensorType`, so a real phone had no way to know it could request `DRIVING_STATUS`/`NIGHT_DATA` — described in research as eager/near-mandatory session-bring-up behavior in `OpenAuto`'s reference implementation (see "Systematic `OpenAuto` diff" above). Implemented `SensorCapability`/`sensor.rs`/`SensorsChannel`, advertising and handling both types. **No change in outcome** — the phone opened the Sensors channel but never sent a `SensorRequest` at all, and the rest of the run matched the pre-Sensors baseline exactly. Kept in the code (a more complete, independently-correct `ServiceDiscoveryResponse` regardless).

The fifth hypothesis (proactive `PingRequest`, "Ping/liveness experiment" above) remains **inconclusive, not refuted** — the probe crashed locally (a new USB bulk-OUT write timeout) before it could be fairly tested; that local failure needed to be worked around (temporarily neutralizing `PING_INTERVAL`) to test the Sensors hypothesis cleanly, but the write-timeout's root cause itself is still unresolved.
...
1. **Immediate next candidates from the `AndroidAutoEntity` control-channel audit** (see "Systematic `OpenAuto` diff" above): `NavigationFocusRequest`/`NavigationFocusResponse` (wire 13/14) and `ShutdownRequest`/`ShutdownResponse` (wire 15/16) both exist and are handled by real `AndroidAutoEntity`, but this project implements neither. Neither has been observed arriving in any real-phone run so far (which would show as an explicit "unexpected control message" rejection, not a silent timeout — their absence from the logs is itself informative, the same reasoning already used to de-prioritize Bluetooth/Microphone), but they're the two remaining named-and-unimplemented control-channel gaps this project now knows about. Implement and test one at a time, same discipline as every prior increment.
2. Diagnose the still-unresolved USB bulk-OUT write timeout from the Ping/liveness experiment (see that section above) — it had to be worked around (temporarily neutralizing `PING_INTERVAL`) rather than resolved to test the Sensors hypothesis cleanly. Candidates worth checking: whether it's specifically the `PingRequest` write that stalls (vs. the next write after it) by adding a print immediately before/after `send_ping_request`'s `send_control` call; whether sending an *unencrypted* frame interleaved with the otherwise-all-encrypted post-handshake traffic disrupts the phone's own USB read loop; or whether this is ordinary real-hardware USB flakiness that happens to coincide with the ~5s mark because the handshake's timing up to that point is itself fairly deterministic.
3. Once the probe can run to a clean completion or timeout again with `PingRequest` active, re-establish whether `Video`/`MediaAudio`/`SystemAudio`/`SpeechAudio` ever receive `Start` on a real run — confirmed `Start` handling is already fully wired into all four dispatch functions, so there is no code gap there; the open question is purely about what the phone sends, not what this project can handle.
4. Given the run-to-run variability observed across this investigation (see "What this rules out, and what it doesn't"), don't over-index on a single run's exact message sequence — repeat any experiment at least twice before drawing conclusions, as this session did for the `SpeechAudio`, Ping, and Sensors real-hardware tests.
5. Look for a newer/different open-source Android Auto receiver project (not necessarily AASDK-derived) that has demonstrably worked with a *current* phone, and diff its full post-service-discovery message sequence against ours.
6. Consider whether a packet capture against a **known-working** current head unit (if one becomes available) could reveal the complete expected message sequence, rather than continuing to discover it one real-phone run at a time.
7. Re-check whether any AASDK fork has been updated since this investigation (2026-08-13) — the ecosystem may catch up.
