...
1. **Done, refuted/no-change:** `ui_config` + USB write instrumentation, the ping-isolation experiment, and a raw `usbmon` capture were all run on real hardware. `ui_config` shows no evidence of helping. The write-timeout question is now resolved at both the "what" (a proactive/unsolicited write of any kind, not `PingRequest` specifically) and "how" (the phone's endpoint never explicitly rejects it — no STALL — it just never answers, and this project's own 10s timeout eventually cancels the transfer) levels. See "Ping isolation experiment" and "`usbmon` capture" above.
2. **Find out *why* the phone's endpoint stops answering proactive writes at some point but keeps answering reactive ones indefinitely.** The best-motivated remaining question for the write-timeout specifically, now that STALL/rejection and ping-content are both ruled out. Candidate angles not yet tried: does inserting a deliberate reactive round-trip (wait for any phone-sent frame) immediately before a proactive send change anything, i.e. is it really about proactive-vs-reactive, or about elapsed time/idle-endpoint duration regardless of who initiates? A longer `usbmon` capture spanning multiple proactive-write attempts (this trial only caught one failure) could show whether the endpoint ever "recovers" and answers a later proactive write, which would distinguish "permanently stops" from "temporarily busy."
3. **Does sending real (or even minimal/synthetic) media data after `Start` change anything?** Still the best-motivated theory for Error 2 itself, independent of the write-timeout question above — see "What this rules out, and what it doesn't". This is real M3 scope (encode/render pipeline: at minimum a single valid, decodable H.264 frame on the video channel, now that `Ack` handling exists to support it), materially larger than any prior increment — plan it as such.
...
5. **Capture a known-good session** (phone → a modern working head unit) if still stalled after the above — compare service discovery response, control-channel frames through `Start`, and first video/audio frames/acks. Highest-value remaining diagnostic if everything above is exhausted, but requires separate hardware/access not currently available.
