...
1. **Diagnose the USB bulk-OUT write timeout with its now-narrower characterization**: it survives LIVI's exact ping arm-timing/cadence (a first, correctly-timed ping succeeded), so the remaining candidates are specifically "a *second* proactive `PingRequest`" or "any outbound write attempted while channel-setup traffic is concurrently in flight" — not general ping timing/framing/transport-timeout, all three of which are now refuted (this trial, plus the two earlier direct fix attempts: encrypted framing, matching AASDK's 10s timeout). A targeted test: try sending a second *non-ping* control message at the same point in the session where the second ping currently fails, to see if the timeout is ping-specific or write-position-specific.
2. **Does sending real (or even minimal/synthetic) media data after `Start` change anything?** Still the best-motivated theory for Error 2 itself (independent of the write-timeout question above) — see "What this rules out, and what it doesn't". This is real M3 scope (encode/render pipeline: at minimum a single valid, decodable H.264 frame on the video channel, now that `Ack` handling exists to support it), materially larger than any prior increment — plan it as such.
3. LIVI is now a formally adopted source (`docs/protocol/livi-adoption.md`) with defined candidate scope beyond what's implemented so far (its full `Session.ts` state machine, `ServiceDiscoveryBuilder.ts`'s `ui_config` population — confirmed present in LIVI, never implemented here). If (1) and (2) don't resolve Error 2, review that candidate scope next, now under the same file-attributed rigor as OpenAuto/AASDK rather than ad hoc comparison.
