...
probe_state=encrypted_frame_received
error=protocol probe: unexpected message on channel 4 after open
```

**Confirmed on the phone screen: still Error 2** ("Communication error 2 — the phone and the car are running incompatible software"), despite the probe now getting substantially further than in every earlier attempt recorded below. This continues to confirm Error 2 is not a single fixed rejection point tied to one specific message — it re-appears at whatever point the session first does something the phone doesn't accept, and that point has now moved from "immediately after `ServiceDiscoveryResponse`" to "after all 8 channels are open, both video and `MediaAudio` have received `Config`, and a `KeyBindingRequest`/`KeyBindingResponse` round trip on the Input channel has completed successfully."

The phone's `KeyBindingRequest` on the Input channel was empty (`key_binding_requested_count=0` — it requested zero keycodes), so the probe's `evaluate_key_binding_request` policy answered `Success` (an empty request has nothing unsupported in it) and the phone accepted that response and moved on, rather than erroring or retrying. The phone's `Setup` on the `MediaAudio` channel was likewise accepted without complaint once answered with a PCM `Config`.

**New failure boundary:** channel 4 (`SystemAudio`) receives a message after `ChannelOpenResponse` that the probe has no handler for — same shape as the previous `MediaAudio` boundary, just one channel later. `ChannelOpenStateMachine` only models advertise→open, nothing past `Open`, for any channel except `Video`/`MediaAudio` (each has their own `Setup`/`Config`/`Start` state machine) and `Input` (has `KeyBindingRequest`/`Response` handling). Byte content of the unhandled message was not logged (no raw payloads, per this project's fail-closed instrumentation policy) — only that it arrived and on which channel. Given `SystemAudio` also derives from AASDK's `AudioMediaSinkService` (same as `MediaAudio`), the same `Setup`/`Config`/`Start` handshake is the leading expectation, though this hasn't been confirmed against the pinned source yet for this specific channel.
...
Implementing that too (`crates/protocol-aap/src/audio_setup.rs`, a deliberate duplicate of `video_setup.rs` rather than a shared generic state machine, to avoid touching the real-hardware-proven video code path — see that file's doc comments for full wire provenance) unblocked the phone yet again: the `Setup` (requesting `MEDIA_CODEC_AUDIO_PCM`) was answered with `Config` and accepted, and the session proceeded to a new message on the `SystemAudio` channel. The pattern established with the first three increments continues to hold exactly: each unblocked boundary reveals precisely one more previously-unseen message elsewhere, and Error 2 keeps re-appearing at whatever the new edge is rather than disappearing.
...
- `Setup`/`Config`/`Start` handling on the `MediaAudio` channel (`crates/protocol-aap/src/audio_setup.rs`, `handle_media_audio_channel_message` in `auth_discovery_probe.rs`) — accepts only `MEDIA_CODEC_AUDIO_PCM`, matching the single uncompressed `AudioConfiguration` this project advertises for `MediaAudio`. Real-hardware-confirmed: the phone's `Setup` requested PCM, the `Config` response was accepted, and the session proceeded past this boundary.
...
1. **Immediate next boundary:** figure out what message the phone sends on the `SystemAudio` channel (channel 4) right after it opens, following the same successful pattern as `AudioFocusRequest`, `KeyBindingRequest`, and `MediaAudio`'s `Setup`/`Config`/`Start`. `SystemAudio` also derives from AASDK's `AudioMediaSinkService` (same base class as `MediaAudio`), so the same `Setup`/`Config`/`Start` handshake is the leading expectation — but confirm this directly against the pinned source before writing any code (check what `AudioConfiguration`/`stream_type` `SystemAudio` advertises in `service_discovery_response.rs` today, since `Setup.type` must match whatever's advertised, same as `MediaAudio`'s PCM). `ChannelOpenStateMachine` has no state past `Open` for `SystemAudio`; this is expected to need its own handler.
2. Given the pattern so far (each unblocked boundary reveals exactly one more previously-unseen message type, and Error 2 keeps re-appearing at the new edge rather than disappearing), expect this to continue: handling the `SystemAudio` message may well reveal a next message on another channel (`SpeechAudio` is architecturally the next most likely, being the third `AudioMediaSinkService`-derived channel). Budget for several more minimal, reversible increments rather than expecting one fix to resolve it.
