...
| Wireless Android Auto bootstrap/session | PX; schema source now identified, not implemented | AASDK's pinned schema (`service/bluetooth`, `service/wifiprojection` incl. `WifiCredentialsRequest`/`Response`) confirmed present, same revision `9bf6adf`; corroborated by `f-io/LIVI` (approved) and `manio/aa-proxy-rs` (GPL-2.0-only, reference only, not adoptable); `docs/protocol/wireless-source-assessment.md` | No; schema-only candidate, M7 unstarted. Raw Bluetooth SDP/pairing handshake (before any AAP channel opens) presumed public-standard, unverified. Checklist item stays open until M7 is real-hardware-confirmed, by explicit project-owner decision — a source existing isn't treated as equivalent to wireless working |
