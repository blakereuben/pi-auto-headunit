...
5. **Ping arm-timing and watchdog model.** Derived from `Session.ts`: the ping timer is armed immediately after `ServiceDiscoveryResponse` is sent (not gated on any later handshake/channel-setup progress), sends `PingRequest` every 1500ms continuously for the session's duration, and a local watchdog closes the session if no `PING_RESPONSE` is seen within 5000ms of the last one. Rust destination: `apps/aa-headunit-diagnostics/src/auth_discovery_probe.rs` (ping scheduling and the currently-neutralized `PING_INTERVAL`/`PROBE_TIMEOUT` logic). Newly adopted by this record; implementation follows.
