...
For an operator-confirmed authorised identity, the diagnostics application may load the protected files through `credential-store` and run a bounded version/TLS interoperability probe. The probe uses project-owned USB identification, emits only sanitised state names, and stops before authentication completion and service discovery. Credential bytes remain external to Git, packages, documentation, test fixtures, logs, and CI artifacts.
