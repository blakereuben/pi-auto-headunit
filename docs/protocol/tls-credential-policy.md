...
The separate `credential-store` layer accepts file paths supplied after installation. It rejects non-regular or oversized files, invalid or mismatched PEM material, certificates outside their validity period, and private keys readable by group or other users. Its installer refuses to overwrite an existing identity. The Debian package creates only an empty protected directory; tests use freshly generated synthetic credentials and never repository secrets. See the [credential provisioning guide](../development/credential-provisioning.md).

For an operator-confirmed authorised identity, the diagnostics application may load the protected files through `credential-store` and run a bounded version/TLS interoperability probe. The probe uses compatibility USB identification with project-specific provenance, emits only sanitised state names, and stops before authentication completion and service discovery. Credential bytes remain external to Git, packages, documentation, test fixtures, logs, and CI artifacts.
