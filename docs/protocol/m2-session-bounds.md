...
**Updated 2026-08-16**: `live_probe.rs` (frozen, unchanged) still only has the wall-clock deadline described below. `auth_discovery_probe::run` and every one of its call sites (`usb`/`developer auth-discovery-probe`, `usb session-supervisor`, `usb gtk-dev-ui`, `usb wireless-bootstrap-probe`) now also check a real, external cooperative-cancellation flag (`apps/aa-headunit-diagnostics/src/cancellation.rs`) each time around the receive loop: `while Instant::now() < deadline && !cancel.is_set()`. The flag is set by a real `SIGINT` (Ctrl-C) handler, installed once per process via the `ctrlc` crate (new workspace dependency, Linux-only target). This closes the specific, narrower gap the M2 checklist item named — "no way to cancel a run early from outside the process" — without pretending it's `ARCHITECTURE.md` §6's future tree-wide, per-channel cancellation-token design: it's one flag, checked in one loop, not yet plumbed through the `app` orchestration layer, which still doesn't exist.

- **A wall-clock deadline, still the primary bound**, unchanged from before — `Instant::now() < deadline` with short-timeout blocking reads (500 ms), checking back roughly twice a second. The cancellation flag is checked at the same cadence, in the same loop condition, not a separate faster-reacting mechanism.
- **Ctrl-C/SIGINT now reaches the loop as a real, observable cancellation** rather than falling through to default Rust/OS process termination: the loop exits the same way it does on a deadline (existing, already-tested resource release below), prints `probe_state=cancelled_by_operator` / `probe_result=cancelled`, and returns the new `CliError::Cancelled` (exit code 22) — distinct from every other outcome, so a caller (a human, or `session-supervisor`'s retry loop, which classifies `Cancelled` as non-retryable and stops the whole supervisor rather than retrying) can tell "the operator asked to stop" apart from a genuine failure. `live_probe.rs` (the frozen `credential-probe`) is deliberately untouched — its own Ctrl-C behaviour is still default process termination, matching its explicit "preserve unchanged" instruction.
- **USB**: `LibUsbBulkTransport` has an explicit `Drop` impl (`transport-usb/src/linux.rs:200`) that releases the claimed USB interface on both a normal return and unwind-driven drop — this already covered the deadline exit path and now covers the cancellation exit path identically, since both leave `run()`'s stack frame the same way.
- **TCP**: `DeveloperTcpTransport`/`WirelessTcpTransport` rely on `std::net::TcpStream`'s default `Drop`, which closes the socket; no custom cleanup, unchanged by cancellation.
- **TLS**: `OpenSslTlsClient` is still dropped with the probe's stack frame; no explicit close-notify is sent on cancellation either — the probe's exit paths (summary received, deadline reached, operator-cancelled, or a protocol/transport error) still none of them send a TLS shutdown. That specific gap (no close-notify) remains open.

For this boundary — short-lived, operator-invoked diagnostic probes, not the long-running appliance session — a hard deadline plus a real external cancel signal, both routed through the same existing, tested `Drop`-based resource release, is what's implemented now. It is still not equivalent to the cooperative, tree-wide, per-channel cancellation `ARCHITECTURE.md` specifies for the eventual `app` layer — that remains future scope, honestly recorded as such, not silently expanded by this change.
