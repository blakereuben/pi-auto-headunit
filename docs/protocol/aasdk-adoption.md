...
`ByeByeRequest`/`ByeByeResponse` are symmetric, not phone-only: `src/Channel/Control/ControlServiceChannel.cpp` (already listed above) defines `sendShutdownRequest` alongside `sendShutdownResponse` and `handleShutdownRequest`/`handleShutdownResponse`, confirming the head unit is meant to be able to initiate a `ByeByeRequest` itself, not only receive and answer one. Confirmed 2026-08-18 after a real gap: this project's probe previously only ever *received* `ByeByeRequest` (`crates/protocol-aap/src/byebye.rs`); ending a session by simply dropping the transport with no wire notice at all left the phone believing the session was still live, so a `session-supervisor` reconnect cycle right after a clean stop collided with the phone still sending encrypted data for the "old" session. `encode_byebye_request` (reason `UserSelection`) is now sent as a best-effort courtesy notice before `auth_discovery_probe::run` returns on a clean, deadline-reached stop.
