...
- [ ] Return calibrated touch input to the phone. **Partially real-hardware-confirmed**: real evdev multitouch capture of the official DSI touchscreen (`platform_api::touch::MultiTouchTracker`, `platform_linux::touch::EvdevTouchSource`), wire-exact `InputReport` encoding (`protocol_aap::encode_touch_report`), and live wiring (`auth_discovery_probe.rs`'s `service_touch_input`) are built and real-hardware-tested across four trials. Single-finger taps are confirmed working end to end — the operator directly confirmed correct on-phone reaction after fixing a coordinate-space mismatch (touch must be advertised/scaled to the negotiated video resolution, not the touchscreen's native panel resolution). Continuous drag (pan) and multitouch (pinch) do not yet register on the phone, and the cause is not yet identified — a second fix (always sending `action_index`, matching `opencardev/openauto`'s reference behavior) was real-hardware-tested and made no difference. See `docs/protocol/touch-input-investigation.md` for the full trial-by-trial record and suggested next steps (most likely: a packet-level capture of a known-working client, the same technique that resolved Error 2).
- [ ] Play media, navigation/system, and speech audio correctly. **Media audio real-hardware-confirmed**: `crates/media-gstreamer/src/audio.rs`'s `AudioPlaybackPipeline` (`appsrc ! audioconvert ! audioresample ! pulsesink`, raw `S16LE` PCM, no decoder stage) is wired into all three AV sink channels (`MediaAudio`/`SystemAudio`/`SpeechAudio` — one independent pipeline instance each, `auth_discovery_probe.rs`'s `MediaPipelines`). A real-hardware trial received 611 real PCM frames on the `MediaAudio` channel (8192 bytes each, ~48kHz/16-bit/stereo) with zero pipeline errors, and the operator directly confirmed correct audible playback through the Pi's USB sound card (PipeWire/`pipewire-pulse`) — not inferred from logs. `SystemAudio`/`SpeechAudio` share the identical code path and format handling but weren't triggered by the phone in that trial (no nav prompt or system sound occurred) — same pipeline, not yet independently observed with real data. See `docs/protocol/error-2-investigation.md`'s audio-playback follow-up section.
