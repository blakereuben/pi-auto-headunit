...
- [ ] Complete session negotiation and service/channel setup. Implementation exists end to end (`ServiceDiscoveryResponse` encoding, `ChannelOpenRequest`/`Response`, video `Setup`/`Config`/`Start`) and is proven correct in real-TLS integration tests (`crates/protocol-aap/tests/full_channel_setup.rs`), but blocked on real hardware: the phone rejects `ServiceDiscoveryResponse` with Android Auto's "Error 2: phone and car are running incompatible software" before ever sending `ChannelOpenRequest`. Three independent hypotheses (missing audio service, offered protocol version, missing head-unit identity) were tested against a real phone and each refuted — see `docs/protocol/error-2-investigation.md` for the full breakdown and open research leads.
