...
- [ ] Complete session negotiation and service/channel setup. Implementation exists end to end (`ServiceDiscoveryResponse` encoding, `ChannelOpenRequest`/`Response`, video/audio `Setup`/`Config`/`Start`, `AudioFocusRequest`, `KeyBindingRequest`, `Sensors`, `NavFocusRequest`, `ByeByeRequest`, an unsolicited `VideoFocusNotification` after video `Config`) and is proven correct in real-TLS integration tests (`crates/protocol-aap/tests/full_channel_setup.rs`). On real hardware, the failure boundary has moved substantially over many increments — most recently, sending the previously-missing unsolicited `VideoFocusNotification` (motivated by comparing against `f-io/LIVI`, a real working independent implementation) got the phone to send `Start` on the video channel for the first time ever, reaching this probe's own internal success condition (`probe_result=video_channel_start_received`). Android Auto's "Error 2: phone and car are running incompatible software" still appears on the phone screen even then — this probe deliberately never sends any real video/audio media data after `Start` (out of scope; that's M3's encode/render pipeline work), so the leading theory is now that Error 2 at this point reflects the complete absence of a following media stream rather than a remaining wire-protocol defect, but this is unconfirmed. See `docs/protocol/error-2-investigation.md` for the full breakdown, all refuted/inconclusive hypotheses, and open research leads.
