...
The default install method below assumes an existing PiOS Desktop. For a dedicated appliance with no desktop underneath (PiOS Lite), see `appliance-lite-flash.sh` (run on the dev machine to prepare the SD card — first boot then goes straight into the guided installer, no manual SSH step) and [`docs/development/pios-lite-appliance.md`](../docs/development/pios-lite-appliance.md) instead — not yet real-hardware-confirmed.

The real, single-file end-user installer is `aa-headunit-installer.sh` (source template) + `build-installer.sh` (appends a built `.deb` to produce the actual distributable file) — self-contained, no separate `.deb` or shortcut needed alongside it, and it doesn't reference any fixed account/path.
